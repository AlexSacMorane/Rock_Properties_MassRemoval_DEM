...
def checkUnbalanced_load_confinement_ic():
    ...
    saveData_ic()
    # check the force applied
    if abs(O.forces.f(plate_x.id)[0] - P_confinement*plate_y.state.pos[1]*plate_z.state.pos[2])/(P_confinement*plate_y.state.pos[1]*plate_z.state.pos[2]) > 0.005 or\
       abs(O.forces.f(plate_y.id)[1] - P_confinement*plate_x.state.pos[0]*plate_z.state.pos[2])/(P_confinement*plate_x.state.pos[0]*plate_z.state.pos[2]) > 0.005 or\
       abs(O.forces.f(plate_z.id)[2] - P_confinement*plate_x.state.pos[0]*plate_y.state.pos[1])/(P_confinement*plate_x.state.pos[0]*plate_y.state.pos[1]) > 0.005 :
        return
    if unbalancedForce() > unbalancedForce_criteria :
        return
    # characterize the ic algorithm
...
def addPlotData_confinement_ic():
    ...
    plot.addData(i=O.iter-iter_0, porosity=porosity(), coordination=avgNumInteractions(), unbalanced=unbalancedForce(), counter_bond=count_bond(),\
                 Sx=sx, Sy=sy, Sz=sz,\
                 conf_verified= 1/3*sx/P_confinement*100 + 1/3*sy/P_confinement*100 + 1/3*sz/P_load*100, \
                 strain_x=100*(plate_x.state.pos[0]-plate_x.state.refPos[0])/plate_x.state.refPos[0],
                 strain_y=100*(plate_y.state.pos[1]-plate_y.state.refPos[1])/plate_y.state.refPos[1],
                 strain_z=100*(plate_z.state.pos[2]-plate_z.state.refPos[2])/plate_z.state.refPos[2])
...
def stopLoad():
    ...
    os.mkdir('../Data_Rock_Triaxial_MassRemoval/'+O.tags['d.id']+'/data')
    shutil.copy('data/'+O.tags['d.id']+'.txt','../Data_Rock_Triaxial_MassRemoval/'+O.tags['d.id']+'/data/'+O.tags['d.id']+'.txt')
    shutil.copy('data/IC_'+O.tags['d.id']+'.txt','../Data_Rock_Triaxial_MassRemoval/'+O.tags['d.id']+'/data/IC_'+O.tags['d.id']+'.txt')
    os.mkdir('../Data_Rock_Triaxial_MassRemoval/'+O.tags['d.id']+'/plot')
    shutil.copy('plot/'+O.tags['d.id']+'.png','../Data_Rock_Triaxial_MassRemoval/'+O.tags['d.id']+'/plot/'+O.tags['d.id']+'.png')
    shutil.copy('plot/IC_'+O.tags['d.id']+'.png','../Data_Rock_Triaxial_MassRemoval/'+O.tags['d.id']+'/plot/IC_'+O.tags['d.id']+'.png')
    shutil.copy('Rock_Triaxial_MassRemoval_batch.py','../Data_Rock_Triaxial_MassRemoval/'+O.tags['d.id']+'/Rock_Triaxial_MassRemoval_batch.py')
...
def saveData():
    ...
    if len(lines) >= 3:
        for i in range(len(data)):
            ...
            L_shear_strain.append(1/2*2/3*(L_strain_z[-1]-L_strain_x[-1]) + 1/2*2/3*(L_strain_z[-1]-L_strain_y[-1]))
