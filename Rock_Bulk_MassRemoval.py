...
def checkUnbalanced_load_confinement_ic():
    ...
    i_load = 1
    n_try_i_load = 0
    P_load = P_load + i_load/n_load * d_P
    L_unbalanced_ite = []
    L_confinement_x_ite = []
    L_confinement_y_ite = []
    L_confinement_z_ite = []
...
def stopLoad():
    ...
    # write in the report
    simulation_report = open(simulation_report_name, 'a')
    simulation_report.write(str(count_bond())+" contacts cemented initially\n\n")
    simulation_report.close()
    ...
    # save simulation
    os.mkdir('../Data_Rock_Bulk_MassRemoval/'+O.tags['d.id'])
    shutil.copytree('data','../Data_Rock_Bulk_MassRemoval/'+O.tags['d.id']+'/data')
    shutil.copytree('plot','../Data_Rock_Bulk_MassRemoval/'+O.tags['d.id']+'/plot')
    shutil.copy('Rock_Bulk_MassRemoval.py','../Data_Rock_Bulk_MassRemoval/'+O.tags['d.id']+'/Rock_Bulk_MassRemoval.py')
    shutil.copy(O.tags['d.id']+'_report.txt','../Data_Rock_Bulk_MassRemoval/'+O.tags['d.id']+'/'+O.tags['d.id']+'_report.txt')
